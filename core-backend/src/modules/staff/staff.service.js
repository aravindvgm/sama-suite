...
// CREATE
// ============================================================
async function createStaff({ organizationId, userId, first_name, last_name }) {
  const { rows } = await pool.query(
    `INSERT INTO staff (organization_id, first_name, last_name, created_by)
     VALUES ($1, $2, $3, $4)
...
// ============================================================
async function getStaff({ organizationId, page, limit, q }) {
...
// GET BY ID
// ============================================================
async function getStaffById({ organizationId, id }) {
  const { rows } = await pool.query(
    `SELECT id, organization_id, first_name, last_name, created_by, created_at, updated_at
     FROM   staff
     WHERE  id              = $1
...
// UPDATE
// ============================================================
async function updateStaff({ organizationId, id, first_name, last_name }) {
  const { rows } = await pool.query(
    `UPDATE staff
...
// SOFT DELETE
// ============================================================
async function deleteStaff({ organizationId, userId, id }) {
  const { rows } = await pool.query(
    `UPDATE staff
